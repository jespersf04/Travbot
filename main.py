...
def compress_image(raw: bytes, max_dim=1600, quality=82):
    """Krymp och komprimera bild sa den aldrig blir for stor for API:t.
    Returnerar (base64_string, media_type). Faller tillbaka pa originalet vid fel."""
    try:
        img = Image.open(io.BytesIO(raw))
        # Konvertera till RGB (hanterar PNG med alfa, HEIC-konverterade m.m.)
        if img.mode in ("RGBA", "P", "LA"):
            ...
            img = img.convert("RGB")
        # Skala ner om nagon sida ar storre an max_dim (behaller proportioner)
        w, h = img.size
        if max(w, h) > max_dim:
        ...
        # Om Pillow inte klarar bilden: anvand originalet okomprimerat
...
def extract_json(text):
    ...
    # Forsok rakt av forst
    try:
        return json.loads(clean)
    except Exception:
        pass
    # Annars: hitta forsta { och matchande sista }
    start = clean.find("{")
    end = clean.rfind("}")
...
async def scan_image(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        for f in files:
            ...
            b64, media_type = compress_image(image_data)
